...
type CellValueProps<R> = Pick<CellContentProps<R>, 'rowIdx' | 'rowData' | 'column' | 'value' | 'isScrolling'>;
/**
 * 根据CellFormatter渲染单元格的值
 */
export default function CellValue<R>({ rowIdx, rowData, column, value, isScrolling }: CellValueProps<R>) {
  function getFormatterDependencies(row: R) {
    ...
    if (getRowMetaData) {
      if (process.env.NODE_ENV === 'development') {
        console.warn(
          'getRowMetaData for formatters is deprecated and will be removed in a future version of ReactDataGrid. Instead access row prop of formatter',
        );
      }
  ...
  if (isValidElementType(formatter)) {
    ...
    return React.createElement(formatter as any, { ...getFormatterProps(), value: value as any }); // FIXME: fix value type
  }

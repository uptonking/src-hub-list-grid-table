...
type SharedViewportProps<R> = Pick<
  ViewportProps<R>,
  | 'rowKey'
  | 'totalWidth'
  | 'rowGetter'
...
export default class Canvas<R> extends React.PureComponent<CanvasProps<R>> {
  ...
  interactionMasks = React.createRef<InteractionMasks<R>>();
  /** 字典，存放行号和对应的行组件 */
  rows = new Map<number, RowRenderer<R> & React.Component<RowRendererProps<R>>>();
  /** 取消订阅 */
  unsubscribeScrollToColumn?(): void;
  /** 包含scrollLeft和scrollTop的对象 */
  _scroll = { scrollTop: 0, scrollLeft: 0 };

  componentDidMount() {
    // 组件首次挂载完成后，会添加订阅滚动到指定列的事件
  ...
  getRowByRef = (i: number) => {
    ...
    if (!this.rows.has(i)) return null;
  ...
  /**
   * just renders empty cells,if to show gridlines, we'd need classes and position as with renderScrollingPlaceholder
   * @param key
   * @param height
   */
    ...
    return (
      <div key={key} style={{ height }}>
        {this.props.columns.map(column => (
          <div style={{ width: column.width }} key={column.key as string} />
        ))}
      </div>
    );
  }
  ...
  renderRow(props: RendererProps<R>) {
    ...
    // 一般执行这里
    // console.log('before renderRow');

    return <Row<R> {...props} />;
  }

  render() {
    console.log('====props4 Canvas');
    ...
    // 获取必需的最少行数的行组件数组
    const rows = this.getRows(rowOverscanStartIdx, rowOverscanEndIdx).map(({ row, subRowDetails }, idx) => {
    ...
    // FIXME react-grid-Canvas的样式包含overflow-y:scroll，始终显示滚动条体验并不好
    return (
      <div
      ...
      >
        {/* InteractionMasks作为独立的组件专注于处理交互事件 */}
        <InteractionMasks<R>

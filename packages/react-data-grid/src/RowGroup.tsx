...
const RowGroup = forwardRef<HTMLDivElement, Props<any>>(function RowGroup(props, ref) {
  function onRowExpandToggle(expand?: boolean) {
    ...
    if (onRowExpandToggle) {
      const shouldExpand = expand === null || expand === undefined ? !props.isExpanded : expand;
...
const DefaultBase = forwardRef<HTMLDivElement, DefaultBaseProps>(function DefaultBase(props, ref) {
  ...
  return (
    <div className='rdg-row-group-default' style={{ height }} onKeyDown={onKeyDown} tabIndex={0} ref={ref}>
      <span className='row-expand-icon' style={{ marginLeft }} onClick={onRowExpandClick}>

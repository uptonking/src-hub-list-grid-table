...
export function getSelectedDimensions<R>({
  selectedPosition: { idx, rowIdx },
  columns,
  rowHeight,
  scrollLeft,
}: getSelectedDimensionsOpts<R>): Dimension {
  if (idx < 0) {
...
export function getSelectedRangeDimensions<R>({
  selectedRange: { topLeft, bottomRight },
  columns,
  rowHeight,
}: getSelectedRangeDimensionsOpts<R>): Dimension {
  if (topLeft.idx < 0) {
...
export function isSelectedCellEditable<R>({
  enableCellSelect,
  selectedPosition,
  columns,
  rowGetter,
  onCheckCellIsEditable,
}: isSelectedCellEditableOpts<R>): boolean {
  const column = columns[selectedPosition.idx];
  const row = rowGetter(selectedPosition.rowIdx);
...
export function getNextSelectedCellPosition<R>({
  cellNavigationMode,
  columns,
  rowsCount,
  nextPosition,
}: getNextSelectedCellPositionOpts<R>): NextSelectedCellPosition {
...
export function canExitGrid<R>(
  event: React.KeyboardEvent,
  { cellNavigationMode, columns, rowsCount, selectedPosition: { rowIdx, idx } }: canExitGridOpts<R>,
): boolean {
